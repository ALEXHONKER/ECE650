...
//     suseconds_t tv_usec;    /*** microsecond 微妙***/
// };
int getint(char *s){
	int i = 0;
	int p = 1;
 ...
 int main(int argc,char *argv[]){
	...
	while(getline(&name,&maxl,fp)!=EOF){
		...
		if(name[0]=='g'){// always g????????????????????????????????????????????????????????????????
			...
			struct timeval t1;
			int kl=gettimeofday(&t1, NULL);
			struct timeval  t2;
    		kl=gettimeofday(&t2, NULL);
    		int first=0;
			...
			close(fd11[0]);
			close(fd22[1]);
			char *st=malloc(sizeof(char)*maxl*1000);
			st[0]='\0';
			strcat(st,pyout2);
			strcat(st,command_path);
			write(fd11[1],st,strlen(st)+1);
			//write(fd11[1],command_path,strlen(command_path)+1);
			free(st);
